...
public class TestServer {
    private Map<Socket, String> playerCards = new HashMap<>(); // 클라이언트 소켓과 카드 매핑
        ...
        public void run() {
            try {
                while (true) {
                        ...
                        synchronized (gameMatches) {
                            if (gameMatches.containsKey(gameName)) {
                                ...
                                if (opponentSocket != null) {
                                    ...
                                    String opponentCard = String.valueOf(random.nextInt(10) + 1); // 상대 카드
                                    String playerCard = String.valueOf(random.nextInt(10) + 1); // 플레이어 카드
                                    playerCards.put(clientSocket, playerCard);
                                    playerCards.put(opponentSocket, opponentCard);

                                    // 카드 정보 전송
                                    sendMsgToClient("카드 정보: " + playerCard, clientSocket);
                                    sendMsgToClient("카드 정보: " + opponentCard, opponentSocket);
                                }
                            } else {
                                // 대기열에 현재 클라이언트 추가
                    ...
                    } else if (msg.startsWith("카드 공개: ")) {
                        // 카드 공개 메시지 처리
                        String cardNumber = msg.substring(6).trim();
                        for (Socket socket : clientSockets) {
                            if (!socket.equals(clientSocket)) {
                                sendMsgToClient("카드 공개: " + cardNumber, socket);
                            }
                        }
                    } else {
                        // 일반 메시지 처리
